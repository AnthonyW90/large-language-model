...
def test_tokenizer(tokenizer):
    """
    Test the tokenizer with sample strings and display statistics.

    Args:
        tokenizer: A trained BPE tokenizer instance
    """
    print("\n" + "=" * 50)
...
def train_bpe_tokenizer(
    ...
    """
    Train a BPE tokenizer on Gutenberg corpus or load existing one.

    Args:
        gutenberg_dir: Path to directory containing Gutenberg text files
        num_files: Number of files to use for training (None for all)
    ...
    if not corpus:
        raise ValueError("No files loaded. Please check the directory path.")
